...
import { Wallet, Calculator, CheckCircle2, DollarSign, Lock } from "lucide-react";
...
interface PayrollSnapshotProps {
  /**
   * Server-rendered seed (admin home batches all snapshot data in a
   * single Promise.all). When omitted (legacy callers / non-financial
   * viewers), the widget falls back to a client-side fetch.
   */
  initialData?: SnapshotData | null;
  /**
   * Render the financial-access empty state instead of fetching. Set
   * by the admin home when the viewer's `financial_access` is false so
   * the 3-up grid keeps its layout without leaking the underlying
   * payroll figures.
   */
...
export function PayrollSnapshot({
  ...
  const [loading, setLoading] = useState(!hideFinancial && initialData === null);

  useEffect(() => {
    if (hideFinancial) return;
    if (initialData) return; // seed is enough on first paint
    getPayrollSnapshot()
      .then(({ data }) => setData(data))
      .catch(() => {})
      .finally(() => setLoading(false));
  }, [hideFinancial, initialData]);
      ...
      <CardContent>
        {hideFinancial ? (
          <div className="flex items-center gap-2 py-3 text-sm text-muted-foreground">
            ...
            <span>Financial visibility is restricted on this account.</span>
          </div>
        ) : loading ? (
