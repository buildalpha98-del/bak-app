...
- **Victorian Curriculum is not modelled.** The stage names, code prefixes and `NSW_STAGES` are NSW-only; VIC (levels, `VC2…` codes) is planned as a framework layer over the registry with a per-centre setting.
