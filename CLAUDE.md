...
- **What the AI is (not) fed.** No syllabus text, exemplar programmes or Scope & Sequences are retrieved; outcome codes and titles come from the model's own recall, steered only by the framework's code-shape guidance, and returned codes are not validated against a source of truth. A curriculum knowledge base (every outcome / content description per subject and band, injected per request and used to validate output) is the next step for audit-grade codes.
