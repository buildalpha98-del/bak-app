...
- **P4** (drag-and-drop scheduling + colour coding + mobile polish) — colour coding **shipped** (migration 072 `centres.colour`; roster Sport|Centre toggle at `?colour=centre`; `lib/utils/centre-colours.ts`; picker in centre settings). Drag-and-drop is **fully wired** in the staff and calendar views (`lib/sessions/dnd-actions.ts`, `dndEnabled` for admin/ops); mobile polish still pending.
