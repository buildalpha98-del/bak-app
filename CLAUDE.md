...
- **P4** (drag-and-drop scheduling + colour coding + mobile polish) — colour coding **shipped** (migration 072 `centres.colour`; roster Sport|Centre toggle at `?colour=centre`; `lib/utils/centre-colours.ts`; picker in centre settings). Drag-and-drop is **fully wired** in the staff and calendar views (`lib/sessions/dnd-actions.ts`, `dndEnabled` for admin/ops). **Mobile polish shipped** (Sept 2026, staff-side Phase 3) — see "Ops on a phone" below.
- **Assign, publish and chase a whole term** (Sept 2026, no migration) — the term board's action strip: **AI assign** (one `/api/scheduling/generate` run per remaining week, `keepExisting`, so every run is on the record), **Publish** (`lib/roster/term-actions.ts` `publishTerm`), **Remind** (`remindUnconfirmed`, one message per coach, `getUnconfirmedByCoach` names who). Two production bugs fixed on the way, both of the "query errors, code reads `data ?? []`" class: (1) **the AI solver never had any coaches** — `compliance_docs` has two FKs to `profiles` (coach + verifier), so `profiles.select("…, compliance_docs(…)")` was refused by PostgREST as ambiguous, `coaches` was null, and every run since launch said "No coaches available"; it now names `compliance_docs!compliance_docs_user_id_fkey` and throws on error. **Name the FK on any embed of `compliance_docs`, `session_coaches`, `attendance` or anything else with two paths to `profiles`.** (2) **Published shifts could not be confirmed** — the status model is `draft → published → pending_confirmation → confirmed` and `confirmShift` accepts only `pending_confirmation`, but nothing ever moved a shift there: every publish path (week, term, scheduling run) left shifts at `published`, coaches were told "you have N shifts to confirm" and could not, and ops bulk-confirmed on their behalf. Publishing now sends a shift **with a coach** to `pending_confirmation` (coach notified) and one **without** to `published` (visible to the centre, nothing to confirm yet) — `publishTerm`, `publishDraftSessionsForWeek` and `publishSchedulingRun` all agree. Also true of the data: **most real coaches have no `availability_slots` rows** (P1's Mon–Fri default only seeded coaches created after it — Abz has 1, Rami/Carla/Amani none), so the solver rightly finds them ineligible; seed or capture availability before expecting AI assign to place the real team.
- **Term setup + term board** (Sept 2026, no migration). The usage numbers before this: Term 3 2026 was rostered by hand (~75 real sessions, typed in one at a time) and attached to the *Term 1* row because that was the current term when they were created; nothing was programmed, the AI was never run, Term 4 was empty 18 days out. So: **read the roster by date window, never by `term_id`** — `lib/terms/term-setup.ts` `inferWeeklyPattern` groups last term's sessions (centre × weekday × time × sport × length; usual coach = mode; regular = ran ≥40% of weeks, else occasional; flags `odd_time` (a 02:30 that was 14:30), `weekend`, `long` (all-day carnival)), and `plannedSessionDates` is the whole-term calendar (clipped to the term, minus skip dates — `NSW_PUBLIC_HOLIDAYS` is a hand-kept map, extend per year — minus what exists). `/{admin,ops}/roster/terms/[id]/setup` (`getTermSetup` finds the last *earlier term that ran anything*; `applyTermSetup` dedupes on centre+weekday+time+sport; `generateSessionsForTerm` makes every week as drafts in one go, idempotent). The roster's **Generate Week was inert** — `templateCount={0}` hard-coded — now the real count, and its empty state links to Set up. **Term board** `/{admin,ops}/roster/coverage?term=`: `lib/roster/coverage-model.ts` `buildCoverageBoard` (pure) — centre × week cells `none | unassigned | draft | unconfirmed | ready | cancelled` (worst thing first), programmed dot, a centre that ran last term with nothing this term sorts first as a gap, and one number: **readiness = % of live sessions both confirmed and programmed**. Cells link to `roster?week=&centre=`. Still true and worth fixing when convenient: the Term-3-dated sessions carry `term_id` = Term 1 2026 (reports/RLS by term_id see them wrongly); a `UPDATE sessions SET term_id = <T3> WHERE date BETWEEN '2026-07-20' AND '2026-09-25' AND term_id = <T1>` would put them right.
...
- **The second coach can see a shared shift** (099, Sept 2026): since 048 `sessions.coach_id` is the **lead only**; every coach on a shift (lead included) has a `session_coaches` row. Nine coach RLS policies and ~35 "my shifts" queries still asked `coach_id = auth.uid()`, so a second coach had no shift, no session plan, no attendance list, no thread — 068 had let them UPDATE a session they could not SELECT. Three real shared shifts never reached their second coach. 099 adds SECURITY DEFINER `auth_coach_session_ids()` / `auth_coach_centre_ids()` (same bypass-RLS pattern as 075, so no recursion) and rewrites the policies on `sessions`, `session_coaches` (any coach on a shift sees the whole crew), `programs`, `session_attendances` ×3, `shift_threads` (WITH CHECK now also requires being on the shift), `centre_children` ×2, `children`, `assessment_templates`, `centre_reports`. App side: **`lib/sessions/coach-membership.ts` is the one way to ask** — `MEMBERSHIP_JOIN` (an *aliased* inner join, so it never disturbs a `session_coaches(...)` embed listing the crew) + `.eq(MEMBERSHIP_FILTER, coachId)` for queries, `isCoachOnSession` for action gates; CI guard `lib/__tests__/no-lead-only-coach-session-reads.test.ts` fails the build on `.from("sessions")….eq("coach_id", …)`. Any coach on the shift can open it, run it (start / progress / complete), edit notes, use the thread and mark attendance, and it counts toward their stats and auto-DND. **Lead-only by design:** confirm/decline and swap requests (the session screen hides those buttons and says who leads). **Pay — the second coach is paid their own rate** (Sept 2026, no migration): a shift stores ONE rate, `sessions.pay_rate_resolved`, the lead's (008's trigger resolves it from `coach_id`; `pay_rate_override` is an override of the *lead's* pay). `lib/pay-rates/coach-shift-pay.ts` `priceShiftForCoach` is **the single pricing rule**: lead = the stored rate with the unit read from their rate table (the long-standing arithmetic, kept to the cent); second coach = `resolvePayRate` with the override nulled → their session-type rate effective on the day, else their default; no rate = a visible $0 line. The weekly payroll batch already paid this way (it now calls the function); the coach's earnings card, uninvoiced-shifts list / self-generated invoice and completed-shifts list now do too, listing every shift the coach was ON via `MEMBERSHIP_JOIN_WITH_ROLE` + `isLeadOf(row)`. Invoiced-already is keyed per (coach, session), so a shared shift sits on both coaches' invoices once each. A second coach never receives the lead's rate: list rows have the pay fields blanked server-side (`withoutLeadPay`) and the session screen fills in the viewer's own rate. (RLS is row-level, so the column itself is still readable on the row by a determined second coach — acceptable, noted.) Any coach on a shift may request an hours adjustment: how long it ran is a fact about the shift. The CI guard has **no exemptions** now. **Admin side counts the whole crew** (Sept 2026, no migration): `CREW_EMBED` / `CREW_JOIN` + `.in(CREW_FILTER, ids)` + `crewOf(row)` in the same helper (with a filtered join `crewOf` is *only the matching coaches* — exactly the attribution an aggregate wants; it falls back to the lead column when there is no embed). Applied to staff hours / next / last shift, the "not rostered" pulse, roster clash detection (`sessionCrewIds` — a second coach's double-booking, travel clash or expired WWCC now raises a warning), availability + swap candidates (a coach working as second no longer shows as free; the crew is excluded from its own replacement list), weekly utilisation, compliance warnings per shift, 24-hour reminders (the second coach gets one), intelligence utilisation + cost-per-centre, the launch dashboard, AI-scheduling familiarity and existing commitments, and the revenue forecast's coach cost (lead's stored rate **plus** each second coach via `priceShiftForCoach`). Deliberately still the lead: "unconfirmed shifts" (only the lead confirms) and coach-name *display* on client surfaces. **Two wrong-column bugs found on the way** — `session_coaches` and `availability_slots` key the coach as `user_id`, but the roster travel-time check and the utilisation pulse asked for `coach_id`: PostgREST errors, the code reads `data ?? []`, and the feature silently does nothing (no travel warning ever fired; every coach was assumed to have 5 slots a week). The CI guard now also fails on `.in("coach_id", …)` against `sessions` and on `coach_id` in a chain on either of those tables. Mocks cannot catch a bad column or embed — after changing a query shape, run it once against the real database. A coach reads only their own `profiles` row, so crewmates' names come via the admin client after the membership gate (names only). `e2e/second-coach.spec.ts` pins it with three throwaway coaches (lead, second, bystander). A streamed `notFound()` returns **200** — assert what renders, not the status.
...
- **Centre delivery log** (Sept 2026, no migration) — the plain record behind the client-facing term report (`lib/reports`): every session of a term for a centre, by DATE window, with crew, programme title, outcome codes, headcount and actual minutes. `lib/centres/delivery-log.ts` (pure: `buildDeliveryLog`, `deliveryLogCsv`), `delivery-log-actions.ts`, `DeliveryLogPanel` at the top of the centre page's Sessions tab (term picker, totals, Download CSV), `GET /api/centres/[id]/delivery-log?term=` (staff session required; 401 otherwise). Delivered = completed or in progress; minutes prefer `actual_duration_minutes`. Note `compileReportData` still reads sessions by `term_id` — which is why the Term-3 rows mislabelled as Term 1 were re-attached on 2026-09-25 (79 rows).
- **`sessions.term_id` follows the date** (100): a BEFORE INSERT / UPDATE OF date trigger sets `term_id` to the term whose window contains the date (a date inside no term keeps what it was given — holiday drafts are deliberate; overlapping terms → earliest wins). Until this, term_id was whichever term was current when the row was typed in, so Term 3's roster sat under Term 1 and every read keyed by term_id (term report, portal session list and impact numbers, coach term stats, public stats cache — 18 sites) was wrong for it; 79 rows were re-attached by hand on 2026-09-25 and 15 more by this migration's backfill. Reads by `term_id` are honest now; reads by date window remain the convention for anything that must also see holiday-dated rows. `e2e/session-term.spec.ts` pins it. If a term's dates are edited, re-run the migration's backfill UPDATE.
- **Ops on a phone** (Sept 2026, no migration) — Abdul runs the day from his phone. Below `sm` the roster opens in the **list** view (decided in a mount effect, never in the `useState` initialiser — the server renders the desktop default and a disagreeing client initial state is a hydration mismatch); `?view=` remains the durable choice. The view/colour toggles, drag hint (`[@media(pointer:fine)]`), Confirm week and the header's Term board / View Terms are desktop chrome; the roster's More menu carries them on phones. The list view's own filter bar is hidden below `sm` (it duplicated the roster chips). `SheetContent` is full width below `sm`. The term board's KPI tiles are two-up with the readiness tile spanning. **Tap targets**: `app/globals.css` gives every `button` / `[role=button]` / `a.inline-flex` / `select` / text `input` inside `main` a 44px `min-height` under `@media (pointer: coarse)` — mouse users keep the 28–32px `size="sm"` buttons. Two gotchas: Turbopack served a stale compiled stylesheet after the globals.css edit (the rule was absent from the served CSS with no error) — restart the dev server if a global rule "doesn't apply"; and Playwright's `devices["iPhone 13"]` carries `defaultBrowserType: "webkit"`, so `test.use` needs `browserName: "chromium"` alongside it. `e2e/ops-mobile.spec.ts` pins list-by-default, the full-width sheet, zero sub-44px controls on `/ops` + `/ops/roster` + the board, and the two-up tiles.
- **Coach availability, self-service** (Sept 2026, no migration): `components/coach/availability-editor.tsx` on `/coach/profile` (first card — without it the roster cannot place the coach), `lib/coach/availability-actions.ts` `getMyAvailability` / `saveMyAvailability` (replace-all, one window per day, RLS `coach_own_availability`). Only staff could enter slots before, from the coach's staff record, so most of the real team had none and the solver rightly found them ineligible. Days: 1 = Mon … 6 = Sat, **Sunday stored as 0** (column CHECK 0–6) while the solver's session side uses 7 — `getEligibleCoaches` now compares mod 7. `e2e/coach-availability.spec.ts` pins the whole loop: no slots → not eligible; ticks Mon–Fri on the profile → eligible.
- **Term programming card** (Sept 2026, no migration) — top of `/{admin,ops}/programs`: `lib/programs/term-programming-actions.ts` `getTermProgrammingStatus` reads the ACTIVE term by date window — sessions vs programmed, a **dry run of `autoProgrammeTerm`** for what the library would attach right now, the groups it has nothing for (each a "Generate a 10-week series" link that opens the generator prefilled via `?sport=&bands=&weeks=` — `ProgramGenerateForm.initial`), approved PDHPE plans and how many of their sessions are still to write (link to the term board), and coach feedback on this term's programmes rolled up per programme (`session_program_feedback` joined to session date; 0 rows existed at build time — the panel explains how ratings arrive). The Auto-programme button here is the same `autoProgrammeTerm` apply as the roster dialog.
- **"Needs your decision" on the ops dashboard** (Sept 2026, no migration) — `lib/ops/decision-queue.ts` (pure shapes + parsing) / `decision-actions.ts` / `components/ops/decision-queue-widget.tsx`, first widget in the command centre's left column, count on the ops pulse strip. Three asks that had no staff screen: **hours adjustments**, **sessions flagged for review** (`needs_ops_review`: ran >15 min long, or the 23:30 cron closed a shift nobody checked out of — 14 of those had sat unsettled since July), and **centres' change requests** (previously visible only inside one session's roster sheet). Each row acts inline: approve/reject with editable minutes; settle at N min or "keep rostered" (plus "settle all at rostered" for the auto-closed batch); approve/decline a move or cancellation. **The hours-adjustment feature had four breaks**, which is why no request ever arrived: the coach's form (`components/pay-rates/hours-adjustment.tsx`) was mounted on no page (now on `/coach/invoicing`); the request insert set a `status` column `tasks` no longer has and no `column_id` it requires (**tasks has no status — the board's columns are the status, `task_columns.is_final` closes**); a coach has no INSERT policy on `tasks`, so the write now goes through the admin client after the coach's own checks (the portal pattern); and approve/reject had no caller. A request is a task titled `Hours adjustment: …` with a "Key: value" description that `parseHoursAdjustmentTask` reads back — if that format changes, change both. Realtime: the command centre also subscribes to `session_change_requests` and task inserts.
