...
import { computeClassRollups, type ClassRollup } from "@/lib/schools/class-rollups";
...
export async function getImpactDashboard(centreId: string) {
  ...
  // Shared with the term-report compiler via computeClassRollups.
  let classRollups: ClassRollup[] = [];
  if (activeTerm) {
    const termSessionIds = (allSessions ?? [])
      .filter((s) => s.date >= activeTerm.start_date && s.date <= activeTerm.end_date)
      .map((s) => s.id);
    classRollups = await computeClassRollups(supabase, centreId, {
      termId: activeTerm.id,
      prevTermId: prevTerm?.id ?? null,
      termSessionIds,
    });
  }
